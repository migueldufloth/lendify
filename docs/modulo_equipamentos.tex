...
\title{
    \textbf{Lendify — Módulo de Equipamentos} \\
    \large Especificação Técnica: Entidades, Regras e Permissões \\
    \normalsize Segurança da Informação — Avaliação N3
}
\author{
    Miguel Angel Balladares Huertas \\
    \small Branch: \texttt{feature/equipamentos} \\
    \small Repositório: \url{https://github.com/migueldufloth/lendify}
}
\date{Joinville, junho de 2026}
...
\begin{tabular}{T{2.8cm} L{3.2cm} L{2.8cm} L{4.6cm}}
  ...
  criado\_por
    & FK → User
    & Automático
    & Chave estrangeira para o usuário (Admin) que cadastrou o equipamento. Preenchido automaticamente no momento da criação via \texttt{request.user}. \\
  \addlinespace
...
O campo \texttt{criado\_por} deve referenciar \texttt{settings.AUTH\_USER\_MODEL}
para respeitar o modelo de usuário customizado definido no módulo de autenticação.
...
\begin{tabular}{@{}lccc@{}}
  ...
  \midrule
  Listar equipamentos          & Sim (filtrado)        & Sim               & Sim \\
  Ver detalhe de equipamento   & Sim                   & Sim               & Sim \\
  Criar equipamento            & Não                   & Não               & Sim \\
  Editar equipamento           & Não                   & Não               & Sim \\
  Excluir equipamento          & Não                   & Não               & Sim \\
  Alterar status (manutenção)  & Não                   & Não               & Sim \\
  \bottomrule
\end{tabular}
\end{center}
...
\begin{itemize}
  ...
  \item \textbf{status}: deve ser um dos valores permitidos: \texttt{disponivel},
        \texttt{reservado} ou \texttt{manutencao}.
...
\begin{itemize}
  \item \textbf{Autorização por perfil:} todas as rotas de criação, edição e exclusão
        ...
        \texttt{administrador}. Caso contrário, retornar HTTP 403 e registrar o evento em log.
  ...
  \item \textbf{Campo criado\_por:} nunca confiar no valor enviado pelo cliente.
        ...
        disponível em \texttt{request.user} via \texttt{JWTAuthentication}.
